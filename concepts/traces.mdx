...
- **Cost.** One extra `POST /v1/traces` per `evaluate()` (2–5ms in parallel with the audit batch). Storage: a few KB per call, kept for 30 days.
...
1. **Per-org isolation.** Trace blobs are stored with the org_id and only readable through the dashboard's clerk-authenticated endpoints. They are not served on the SDK-direction routes.
2. **DLP redaction (opt-in).** When DLP is enabled, the trace upload endpoint redacts detected secrets/PII before persisting. The full unredacted trace never touches disk.
