...
<Steps>
  ...
  <Step title="Upload trace (if attached)">
    If a `TraceContext` was passed, the SDK appends a `ToolCallMessage` and uploads the running messages array to the control plane. The returned `traceId` and position are attached to the audit event.
  </Step>
  <Step title="Enqueue audit event">
    The event goes into the audit sink's in-memory queue. The flush thread batches up to 100 events or 1 second of latency and ships them to the control plane. **Your code does not wait on the network.**
  </Step>
...
Without traces and without DLP, `evaluate()` finishes in well under 1ms on a modern laptop. With traces it's bounded by your network RTT to the control plane.
