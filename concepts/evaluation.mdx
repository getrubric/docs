...
<Steps>
  ...
  <Step title="Upload trace (if attached)">
    If a `TraceContext` was passed, the SDK appends a `ToolCallMessage` and uploads the running messages array to `/v1/traces`. The returned `traceId` and position are attached to the audit event.
  </Step>
  <Step title="Enqueue audit event">
    The event goes into the audit sink's in-memory queue. The flush thread batches up to 100 events or 1 second of latency and POSTs to `/v1/events`. **Your code does not wait on the network.**
  </Step>
...
- Native (Rust) evaluator (`pip install agent-governance[runtime]`): **~5–20µs**.
