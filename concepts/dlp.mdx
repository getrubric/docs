...
| `presidio` | ~1ms/call | Everything in `regex` plus NER-backed PII (names, locations, organizations, medical) via Microsoft Presidio. |
| `auto` | as available | Presidio if installed (`agent-governance[presidio]`), otherwise regex. |
...
```python
from agent_governance.dlp import DlpDetection
from agent_governance import Governance
