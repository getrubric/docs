...
The `[mcp]` extra pulls `mcp` as a dependency. If you already have it installed, `pip install agent-governance` is enough.

## Basic usage

```python
from agent_governance import Governance
from agent_governance.adapters.mcp import govern_mcp_session
...
A working end-to-end script lives at [`sdk/python/examples/mcp_quickstart.py`](https://github.com/getrubric/platform/blob/main/sdk/python/examples/mcp_quickstart.py) in the platform repo. It spawns an in-tree FastMCP server, governs the client, and runs an `allow` and a `deny` call against the bundle.
