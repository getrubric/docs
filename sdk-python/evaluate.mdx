...
The trace is uploaded to the control plane and the returned `traceId` is attached to the audit event. See [Traces](/concepts/traces).
...
- Native (Rust) evaluator: ~5–20µs (`pip install rubric[runtime]`).
