...
`Governance` owns the agent's signed identity, the bundle cache, the audit sink, and the trace uploader. Most application code should use the [decorator API](/sdk-python/init) instead — `rubric.init()` once, `@rubric.tool` per function. Reach for `Governance` directly when you need:
