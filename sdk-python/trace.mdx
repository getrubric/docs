...
A `TraceContext` is a running list of messages that represents the conversation up to the current tool call. When you pass `trace=trace` to `evaluate()`, the SDK appends a `ToolCallMessage` automatically and uploads the whole list to the control plane. The dashboard's audit drawer renders it on click.
