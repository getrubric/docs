...
The decorator API is the recommended way to use the SDK. `init()` once at process startup, then decorate any function with `@rubric.tool` — every call routes through your policy bundle automatically.
...
Context manager that sets the session id for any `@rubric.tool` calls inside the block. Falls back to `default_session_id` (default `"default"`) outside any scope.
...
| `rubric.GovernanceProblemError` | Control-plane returned an RFC 9457 problem response (e.g. enrollment token revoked). |
