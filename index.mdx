...
ag.init(agent_name="payments-bot")  # once, at process startup
...
Set `AG_ENROLLMENT_TOKEN` once, sprinkle `@ag.tool` over your tool functions, and every call routes through your policy bundle. No `with` blocks, no manual `evaluate()` calls.
