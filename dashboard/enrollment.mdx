...
- Sets `revoked_at = now()`. Future `/enroll` calls with this token return `401 invalid_identity`.
- Existing agent identities created via this token **continue to work** — their JWTs are independent of the enrollment token. You revoke individual identities separately.

To kill both the token *and* every agent it enrolled, use **Bulk revoke** (placeholder in the current build; coming soon).

## Provenance on agents

In the **Identities** section of any agent detail page, identities created via an enrollment token show:

```
enrolled · <tokenName>
```

The chip links back to the token. Identities created via a one-time provisioning secret show `manual`. This is your trail when you need to answer *"how did this agent get its credential?"*.

## Security model recap

Enrollment tokens are **register-only**:

- They cannot ship audit events (those need a JWT-SVID, which only the enroll endpoint mints).
- They cannot read bundles.
- They cannot impersonate existing agents — the only thing they can do is create new agent rows in your org and get a JWT for those new rows.
