...
- **Agent** — name and stable id (the value bound to the JWT's `sub` claim).
...
- **Identities** — every JWT-SVID issued for this agent, with status (active/revoked) and provenance (`enrolled · <token>` or `manual`). Issue or revoke from here.
...
Agents are usually registered automatically — the SDK calls `/v1/identities/enroll` (or `/exchange`) on bootstrap and the control plane creates the agent row idempotently. **You rarely need to register agents manually.**
...
2. The current JWT remains valid until expiry (≤ 60 min).
3. New audit events from this agent are still logged but the dashboard shows them with a "frozen" tag.

To **immediately** stop an agent, revoke its identity (Identities → Revoke). The current JWT is still cryptographically valid until expiry, but you can pair revocation with a kill-switch in your harness if you need stronger guarantees.
